...
RETURNS TABLE(osm_id bigint, geometry geometry, name text, name_en text, name_de text, tags hstore, class text, subclass text, agg_stop integer, layer integer, level real, indoor integer, "rank" int) AS $$
    SELECT osm_id_hash AS osm_id, geometry, NULLIF(name, '') AS name,
        ...
        unnest(array_cat(string_to_array(level, ';'), repeat_on_to_array(repeat_on)))::real AS level,
